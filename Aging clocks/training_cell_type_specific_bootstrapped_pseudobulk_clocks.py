...
###
###
### training cell-type-specific bootstrapped pseudobulk aging clocks
###
###
...
for cell_type in cell_types:
    ...
    for donor in donors:
        ...
        if len(df_aux)>num_cells[cell_type]:
            for j in range(100):
                ...
                df_sample_meta = df_aux[["donor_id", "age", "seq_batch", "cell_type"]].iloc[0]
    ...
    for alpha in alphas:
        ...
        cv_results = cross_validate(model, df_avg[df_avg.columns[:-4]], df_avg["age"], groups = df_avg["donor_id"],
        ...
        for i in range(5):
            ...
            col_names = np.append(list(df_avg.columns[:-4]), "intercept")
            ...
            df_current = df_avg.iloc[cv_results["indices"]["test"][i]][["age", "donor_id", "seq_batch", "cell_type"]]
            predicted = cv_results["estimator"][i].predict(df_avg.iloc[cv_results["indices"]["test"][i]][df_avg.columns[:-4]])
            df_current["predicted_age"] = list(predicted)
            df_final = pd.concat([df_final, df_current])
    
        df_models.to_csv("clocks/pseudobulk_celltype_bootstrapped_"+str(cell_type)+"_"+str(alpha)+".csv", index=False)
        df_final.to_csv("results/predictions_pseudobulk_celltype_bootstrapped_"+str(cell_type)+"_"+str(alpha)+"_.csv")
